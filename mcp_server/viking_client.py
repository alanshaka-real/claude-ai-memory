...
class VikingClient:
    """Wraps OpenViking async SDK with project-aware operations.

    Uses AsyncHTTPClient to avoid event loop conflicts with MCP's FastMCP framework.
    ...
    async def _ensure_connected(self) -> bool:
        """Lazy async initialization of OpenViking client."""
        if self._ov is not None and self._initialized:
            return True
        if AsyncHTTPClient is None:
            logger.warning("openviking package not installed")
            return False
        try:
            self._ov = AsyncHTTPClient(url=self.url, api_key=self.api_key)
            await self._ov.initialize()
            self._ov._is_local_server = lambda: False
            self._initialized = True
            logger.info("Connected to OpenViking")
            return True
    ...
    async def is_available(self) -> bool:
        if not await self._ensure_connected():
            return False
        try:
            # Use ls on root as health check (is_healthy has event loop issues)
            await self._ov.ls("viking://resources")
            return True
        except Exception as e:
            logger.warning(f"is_available: health check failed ({e}), reconnecting")
            self._ov = None
            self._initialized = False
            if not await self._ensure_connected():
                return False
    ...
    async def project_exists(self, project_path: str) -> bool:
        if not await self.is_available():
            return False
        try:
            uri = self.pm.get_project_uri(project_path)
            await self._ov.ls(uri)
            return True
        except Exception as e:
            logger.debug(f"project_exists check failed for {project_path}: {e}")
            return False

    async def init_project(self, project_path: str) -> str:
        ...
        for subdir in ["sessions", "decisions", "changes", "knowledge"]:
            try:
                await self._ov.mkdir(f"{base_uri}/{subdir}")
            except Exception:
                pass  # Directory may already exist
        return pid

    async def save_entries(self, project_path: str, entries: list[MemoryEntry]) -> int:
        saved = 0
        for entry in entries:
            try:
                ...
                with tempfile.NamedTemporaryFile(
                    ...
                    tmp_path = f.name
                try:
                    await self._ov.add_resource(tmp_path, target=target_uri)
                    saved += 1
                finally:
    ...
    async def load_context(self, project_path: str) -> dict[str, Any]:
        if not await self.is_available():
            return {"available": False, "reason": "OpenViking not reachable"}
        if not await self.project_exists(project_path):
        ...
        try:
            result["summary"] = await self._ov.abstract(base_uri)
        except Exception:
            result["summary"] = None

        try:
            sessions_uri = f"{base_uri}/sessions"
            items = await self._ov.ls(sessions_uri, simple=True)
            recent = sorted(items, reverse=True)[:5]
            result["recent_sessions"] = []
            for item in recent:
                try:
                    abstract = await self._ov.abstract(f"{sessions_uri}/{item}")
        ...
        try:
            ...
            for ctx in todo_contexts:
                ...
                if not content:
                    try:
                        content = await self._ov.read(getattr(ctx, "uri", ""))
                    except Exception:
                        pass
                if content:
        ...
        try:
            ...
            items = await self._ov.ls(decisions_uri, simple=True)
            recent_decisions = sorted(items, reverse=True)[:3]
            result["active_decisions"] = []
            for item in recent_decisions:
                try:
                    abstract = await self._ov.abstract(f"{decisions_uri}/{item}")
    ...
    async def search(
        ...
        try:
            find_result = await self._ov.find(query, target_uri=target_uri, limit=limit)
            ...
            for ctx in contexts:
                ...
                if not content and uri:
                    try:
                        content = await self._ov.read(uri)
                    except Exception:
                        pass
                output.append({
    ...
    async def list_entries(self, project_path: str, subdir: str = "") -> list[dict[str, Any]]:
        base_uri = self.pm.get_project_uri(project_path)
        target = f"{base_uri}/{subdir}" if subdir else base_uri
        try:
            items = await self._ov.ls(target)
    ...
    async def delete_entry(self, project_path: str, target_uri: str) -> bool:
        if not self._validate_project_uri(project_path, target_uri):
            return False
        try:
            await self._ov.rm(target_uri)
            return True
        except Exception as e:
            logger.error(f"Delete failed: {e}")
            return False

    async def update_entry(self, project_path: str, target_uri: str, content: str) -> bool:
        if not self._validate_project_uri(project_path, target_uri):
            return False
        try:
            ...
            try:
                ...
                await self._ov.add_resource(tmp_path, target=parent_uri)
            finally:
                Path(tmp_path).unlink(missing_ok=True)
            return True
        except Exception as e:
